...
All IDs are preconfigured — no manual setup needed.
"""
...
async def cmd_about(message: types.Message):
    await message.answer("ℹ️ Бот создан для управления каналом и группой с помощью ИИ. Версия 2.0")
...
async def moderate_group_messages(message: types.Message):
    # Only moderate the specified group
